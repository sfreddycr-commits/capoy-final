...
type Tour={id:number;slug:string;name:string;destination:string;shortDescription:string|null;description:string|null;duration:string|null;adultPrice:number;childPrice:number|null;currency:string;capacity:number|null;mainImageUrl:string|null;status:string;featured:boolean;sortOrder:number|null;publishedAt:string|null;createdAt:string;updatedAt:string;galleryImages?:string[];translations?:Record<string,Translation>};
...
function resolveCardImage(t:Tour):string|null{if(t.mainImageUrl&&String(t.mainImageUrl).trim())return t.mainImageUrl;const g=parseGallery(t.galleryImages);return g[0]||null}
...
export function ToursPage(){
  ...
  async function savePhoto(){if(!editing||!pendingFile)return;const ok=await uploadMainImage(pendingFile);if(ok){setPhotoMode(false);setPendingFile(null);await load()}}
  ...
  {!loading&&!error&&data&&data.tours.length>0&&<div className="tours-grid">{data.tours.map(t=>{const g=parseGallery(t.galleryImages);const cardImg=resolveCardImage(t);return <article key={t.id}><div className="tour-photo-wrap">{cardImg?<img src={cardImg} alt={t.name}/>:<div className="tour-placeholder">Sin imagen</div>}<button className="tour-photo-btn" onClick={()=>openPhoto(t)}><Camera size={16}/> Cambiar foto</button></div><div className="tour-card-body"><div className="tour-card-top"><span className={`tour-status ${t.status}`}>{statuses.find(s=>s[0]===t.status)?.[1]||t.status}</span>{t.featured&&<span className="tour-status featured">Destacado</span>}<button onClick={()=>openEdit(t)}><Edit3 size={16}/> Editar</button></div>
  ...
  {!photoMode&&(<>
    ...
    {(!editing||editing.id)&&<>
      ...
      <label>Destacado<select name="featured" defaultValue={editing?.featured?'1':'0'}><option value="0">No</option><option value="1">Sí</option></select></label>
      <label>Orden<small className="tour-field-hint">Menor valor aparece primero. Vacío = al final.</small><input name="sortOrder" type="number" min="0" max="999999" defaultValue={editing?.sortOrder??''} placeholder="ej. 10"/></label>
